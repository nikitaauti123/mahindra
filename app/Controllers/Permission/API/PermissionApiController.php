...
class PermissionApiController extends BaseController
{
    ...
    public function add()
    {
        ...
        try {
            ...
            $this->permissionsModel->where('is_active', '1');
            $this->permissionsModel->where('deleted_at IS NULL');
            $this->permissionsModel->where('permission_id', $this->request->getVar('permission_id'));
            $res = $this->permissionsModel->find();
    ...
    public function update($id)
    {
        try {
            ...
            if (!$this->validate($rules)) {
             ...
             if ($id != '') {
                $this->permissionsModel->where('id !=', $id);
            }
            $this->permissionsModel->where('is_active', '1');
            $this->permissionsModel->where('deleted_at IS NULL');
            $this->permissionsModel->where('permission_id', $this->request->getVar('permission_id'));
